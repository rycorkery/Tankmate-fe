...
function generateDummyData(days: number = 30): ParameterDataPoint[] {
  ...
  for (let i = days - 1; i >= 0; i--) {
    ...
    if (Math.random() > 0.6) {
      // 40% chance of having nitrate data
      data.push({
        date: dateString,
    ...
    if (Math.random() > 0.7) {
      // 30% chance of having nitrite data
      data.push({
        date: dateString,
    ...
    if (Math.random() > 0.8) {
      // 20% chance of having ammonia data
      data.push({
        date: dateString,
    ...
    if (Math.random() > 0.9) {
      // 10% chance of having salinity data (for saltwater tanks)
      data.push({
        date: dateString,
...
export function ParameterTrends({ tankId }: ParameterTrendsProps) {
  ...
  const [activeParameters, setActiveParameters] = useState<
    Record<CreateParameterRequestType, boolean>
  >(() =>
    Object.keys(PARAMETER_CONFIG).reduce(
      (acc, key) => {
    ...
    () => ({
        ...
        tooltip: {
          ...
          callbacks: {
            label: (context) => {
              ...
              const config = Object.values(PARAMETER_CONFIG).find(
                (c) => c.label === context.dataset.label
              )
        ...
        y: {
          ...
          ticks: {
            ...
            callback: function (tickValue: string | number) {
              ...
              const originalValue =
                normalizedValue * (config.yAxisRange.max - config.yAxisRange.min) +
                config.yAxisRange.min
          ...
          title: {
            ...
            text: hoveredParameter
              ? `${PARAMETER_CONFIG[hoveredParameter].label} (${PARAMETER_CONFIG[hoveredParameter].unit})`
              : '',
            color: '#64748b',
            font: {
              size: 12,
      ...
      onHover: (_, activeElements) => {
        if (activeElements.length > 0) {
          ...
          const dataset = chartData.datasets[datasetIndex] as {
            parameterType?: CreateParameterRequestType
          }
        ...
        {hoveredParameter && (
          <div className="text-sm text-slate-600 bg-slate-50 px-3 py-1.5 rounded-lg">
            <span
              ...
              style={{ color: PARAMETER_CONFIG[hoveredParameter].color }}
            >
            ...
            return (
              <button
                ...
                className={`
                  ...
                  ${hasData ? 'cursor-pointer hover:bg-slate-100' : 'cursor-not-allowed opacity-40'}
                  ${isActive ? '' : 'opacity-60'}
